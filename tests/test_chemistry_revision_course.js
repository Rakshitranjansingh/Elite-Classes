// tests/test_chemistry_revision_course.js
const fs = require('fs');
const path = require('path');
const assert = require('assert');

console.log('================================================================');
console.log('🧪 TESTING CIVIL SERVICES CHEMISTRY REVISION COURSE');
console.log('================================================================\n');
...
assert(revHubHtml.includes('chemistry/chemistry_course_hub.html'), 'Revision hub must link to chemistry/chemistry_course_hub.html');
console.log('✔ revision_course_hub.html verified with chemistry link.');

// 3. Test Chemistry Hub
const hubPath = path.join(__dirname, '../modules/course/civilservices/revisionCourse/chemistry/chemistry_course_hub.html');
assert(fs.existsSync(hubPath), 'chemistry_course_hub.html must exist');
...
assert(hubHtml.includes('cs_chem_ch1'), 'Hub must register cs_chem_ch1');
assert(hubHtml.includes('CHEMISTRY_CHAPTERS'), 'Hub must contain CHEMISTRY_CHAPTERS');
assert(hubHtml.includes('../../../course_player.html'), 'Hub must link to central course_player.html');
console.log('✔ chemistry_course_hub.html verified with central player link.');

// 4. Test All 7 Chapter Course Data Files in chemistry/data/
for (let ch = 1; ch <= 7; ch++) {
    const dataPath = path.join(__dirname, `../modules/course/civilservices/revisionCourse/chemistry/data/chapter${ch}_course_data.js`);
    ...
    assert.strictEqual(chData.passingPercentage, 70, `Chapter ${ch} passingPercentage must be 70`);
...
console.log('\n✨ ALL TESTS PASSED! Civil Services Chemistry Revision Course (140 Modules, 1,400 MCQs) is 100% verified.');
