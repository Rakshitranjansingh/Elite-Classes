// tests/test_medieval_history_revision_course.js
const fs = require('fs');
const path = require('path');
const assert = require('assert');

console.log('================================================================');
console.log('🏰 TESTING CIVIL SERVICES MEDIEVAL INDIAN HISTORY REVISION COURSE');
console.log('================================================================\n');
...
assert(revHubHtml.includes('medievalindia/medievalindia_course_hub.html'), 'Revision hub must link to medievalindia/medievalindia_course_hub.html');
assert(revHubHtml.includes('5 Subjects Live'), 'Revision hub must show 5 Subjects Live');
console.log('✔ revision_course_hub.html verified with medieval history link and 5 Subjects Live counter.');

// 3. Test Medieval India Hub
const hubPath = path.join(__dirname, '../modules/course/civilservices/revisionCourse/medievalindia/medievalindia_course_hub.html');
assert(fs.existsSync(hubPath), 'medievalindia_course_hub.html must exist');
...
assert(hubHtml.includes('cs_med_ch1'), 'Hub must register cs_med_ch1');
assert(hubHtml.includes('MEDIEVAL_CHAPTERS'), 'Hub must contain MEDIEVAL_CHAPTERS');
assert(hubHtml.includes('../../../course_player.html'), 'Hub must link to central course_player.html');
console.log('✔ medievalindia_course_hub.html verified with central player link and clickable logo.');

// 4. Test All 7 Chapter Course Data Files in medievalindia/data/
for (let ch = 1; ch <= 7; ch++) {
    const dataPath = path.join(__dirname, `../modules/course/civilservices/revisionCourse/medievalindia/data/chapter${ch}_course_data.js`);
    ...
    chData.modules.forEach((mod, idx) => {
        ...
        assert(!/\bNCERT\b|\bBCW\b|Sprint\s*72|BPSC\s*Concept\s*Wallah/i.test(fullModText), `Forbidden publisher/brand mentioned in Ch ${ch} Mod ${idx+1}`);
        ...
        mod.questions.forEach((q, qIdx) => {
            ...
            assert(!/\bNCERT\b|\bBCW\b|Sprint\s*72|BPSC\s*Concept\s*Wallah/i.test(q.question + ' ' + q.explanation), `Forbidden publisher mentioned in Ch ${ch} Mod ${idx+1} Q ${qIdx+1}`);
            qCount++;
        });
    });
...
console.log('\n✨ ALL TESTS PASSED! Civil Services Medieval Indian History Revision Course (140 Modules, 1,400 MCQs) is 100% verified.');
