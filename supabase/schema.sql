...
ALTER TABLE test_series ADD COLUMN IF NOT EXISTS test_date VARCHAR(50);
...
ALTER TABLE test_questions ADD COLUMN IF NOT EXISTS question_type VARCHAR(50) DEFAULT 'mcq';
ALTER TABLE test_questions ADD COLUMN IF NOT EXISTS marks NUMERIC(4, 2) DEFAULT 4.00;
ALTER TABLE test_questions ADD COLUMN IF NOT EXISTS negative_marks NUMERIC(4, 2) DEFAULT 1.00;
...
-- =====================================================================
-- 12. DEDICATED TEST SERIES SUBSCRIBERS MODULE
-- =====================================================================
CREATE TABLE IF NOT EXISTS testseries_subscribers (
    id TEXT PRIMARY KEY,
    name TEXT NOT NULL,
...
-- =========================================================================
-- SEED CLASS 10 SOCIAL SCIENCE ASSESSMENTS (HISTORY, POLITICS, ECONOMICS, GEOGRAPHY)
-- =========================================================================
INSERT INTO subjects (id, name, code, description, is_active)
VALUES
...
-- 28. ADDITIONAL EMAIL & GOOGLE AUTH IDENTIFIERS
ALTER TABLE students ADD COLUMN IF NOT EXISTS additional_email VARCHAR(255);
ALTER TABLE staff ADD COLUMN IF NOT EXISTS additional_email VARCHAR(255);
ALTER TABLE admins ADD COLUMN IF NOT EXISTS additional_email VARCHAR(255);

CREATE UNIQUE INDEX IF NOT EXISTS idx_students_unique_email ON students (LOWER(TRIM(email))) WHERE email IS NOT NULL AND email != '';
CREATE UNIQUE INDEX IF NOT EXISTS idx_students_unique_additional_email ON students (LOWER(TRIM(additional_email))) WHERE additional_email IS NOT NULL AND additional_email != '';
CREATE UNIQUE INDEX IF NOT EXISTS idx_staff_unique_email ON staff (LOWER(TRIM(email))) WHERE email IS NOT NULL AND email != '';
CREATE UNIQUE INDEX IF NOT EXISTS idx_staff_unique_additional_email ON staff (LOWER(TRIM(additional_email))) WHERE additional_email IS NOT NULL AND additional_email != '';
CREATE UNIQUE INDEX IF NOT EXISTS idx_admins_unique_email ON admins (LOWER(TRIM(email))) WHERE email IS NOT NULL AND email != '';
CREATE UNIQUE INDEX IF NOT EXISTS idx_admins_unique_additional_email ON admins (LOWER(TRIM(additional_email))) WHERE additional_email IS NOT NULL AND additional_email != '';

-- 29. USER DAILY LOGIN LOGS (PER-USER PER-DAY COUNTER)
CREATE TABLE IF NOT EXISTS user_daily_login_logs (
    ...
    email VARCHAR(255),
    login_method VARCHAR(50) DEFAULT 'whatsapp_pin',
...
BEGIN
    INSERT INTO user_daily_login_logs (
        user_id, user_role, login_date, login_count, email, login_method, last_login_at, ip_address, user_agent
    ) VALUES (
        p_user_id, p_user_role, CURRENT_DATE, 1, p_email, p_login_method, NOW(), p_ip, p_user_agent
    )
    ON CONFLICT (user_id, login_date) DO UPDATE
    SET login_count = user_daily_login_logs.login_count + 1,
        last_login_at = NOW(),
...
CREATE POLICY "Allow authenticated and public insert login logs" ON user_daily_login_logs FOR INSERT WITH CHECK (true);
DROP POLICY IF EXISTS "Allow authenticated update login logs" ON user_daily_login_logs;
CREATE POLICY "Allow authenticated update login logs" ON user_daily_login_logs FOR UPDATE USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Allow public read own login logs" ON user_daily_login_logs;
CREATE POLICY "Allow public read own login logs" ON user_daily_login_logs FOR SELECT USING (true);
